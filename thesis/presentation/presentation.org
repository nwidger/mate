...
**** Tracks blocked threads, wake them during serial mode
      ...
      + Holds threads at end their serial/parallel segment

      + Wake each in creation order during serial mode
...
**** Allocates DMP-specific modules for each =object=, =thread=,
    ...
    #+BEGIN_SRC c
      /* called by object/create/nlock/table_create, pointer stored in 'dmp' field */
...
*** Thread DMP
    ...
    #+BEGIN_SRC c
      struct thread_dmp_ops {
              ...
              int  (*execute_instruction)(struct thread_dmp *td, uint32_t o); /* called when thread executes instruction */
      };
...
*** NLock DMP
    ...
    + Implements object monitors

    #+BEGIN_SRC c
      struct nlock_dmp_ops {
              int  (*lock)(struct nlock_dmp *nd);      /* acquire monitor */
              int  (*unlock)(struct nlock_dmp *nd);    /* release monitor */
              int  (*timedwait)(struct nlock_dmp *nd); /* acquire monitor w/ timeout */
      };
