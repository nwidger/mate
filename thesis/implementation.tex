...
thread using a call to $pthread_setaffinity\_np$.

% -Added integer cache to VM for immutable Integer objects as they are
% created
%
% -Added string cache to VM, permanently stores any strings created
% using the newstr instruction
% 
% -Added object ref cache to VM threads, bypass heap access to
% translate object ref to object pointer.
%
% -Added free object cache to VM threads, store free'd objects by
% size, bypass heap access is free object cache contains a free object
% of the correct size.
% 
% -Added Linux syscall to set thread affinity when running with DMP
% enabled, forces threads to have dedicated CPU.
